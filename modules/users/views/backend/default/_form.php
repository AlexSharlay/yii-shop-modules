 ...
 * @var \common\modules\users\models\backend\User $model Model
...
<?php $box->beginBody(); ?>
    ...
    <hr/>
    <? // ['name','surname','patronymic','firmName','ynp','legal_address','settlement_account','phone_company','phone_director']; ?>
    <div class="row">
        <div class="col-sm-6">
            <?= $form->field($profile, 'name') ?>
        </div>
        <div class="col-sm-6">
            <?= $form->field($profile, 'firmName') ?>
        </div>
    </div>
    <div class="row">
        <div class="col-sm-6">
    ...
    <div class="row">
        ...
        <div class="col-sm-6">
            <?= $form->field($profile, 'legal_address') ?>
        </div>
    </div>
    <div class="row">
        <div class="col-sm-6">
            <?= $form->field($profile, 'phone_director') ?>
        </div>
        <div class="col-sm-6">
            <?= $form->field($profile, 'settlement_account') ?>
        </div>
    </div>
    <div class="row">
        <div class="col-sm-6">
            <?= $form->field($profile, 'phone_company') ?>
        </div>
        <div class="col-sm-6">
            <?= $form->field($profile, 'id_city', ['inputOptions' => ['class' => 'selectpicker ']])
                ->dropDownList(UserCity::getCities(), ['prompt' => '- Город доставки -', 'class'=>'form-control required']); ?>
        </div>
    </div>
